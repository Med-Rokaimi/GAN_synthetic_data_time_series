...
if __name__ == '__main__':
    ...
    df = df[6:]
    df = df[['Price', 'SENT']]
    garch_model = arch_model(df['Price'].values, vol='Garch', p=1, q=1)
    garch_fit = garch_model.fit(disp='off')

    # Print summary of the GARCH model
    print(garch_fit.summary())

    # Forecast the conditional variances (volatilities) for each time step
    garch_forecasts = garch_fit.conditional_volatility

    # Add the forecasted volatility as a new feature to the dataframe
    df['Volatility_Forecast'] = garch_forecasts

    # Ensure no NaN values are present
    df['Volatility_Forecast'].fillna(method='bfill', inplace=True)

    # Verify the length of the new feature
    print(df.shape)  # Should be (2380, 3) with 'Price', 'SENT', 'Volatility_Forecast'
    print(df.head())
