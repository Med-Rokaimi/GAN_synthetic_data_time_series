...
class Generator(nn.Module):
    def __init__(self, noise_size, x_batch_size, generator_latent_size, feature_no):
        ...
        #self.cond_to_latent = nn.LSTM(input_size=feature_no,
                                     #hidden_size=generator_latent_size)

        self.cond_to_latent = nn.LSTM(
            self.input_dim, self.generator_latent_size, 1, batch_first=True, dropout=0.33
        )

        self.model = nn.Sequential(
            nn.Linear(in_features=self.generator_latent_size + noise_size,
                      out_features=self.generator_latent_size + noise_size),
            nn.ReLU(),
            nn.Linear(in_features=self.generator_latent_size + noise_size, out_features=1)
        )
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load("./results/forGAN_LSTM/best.torch")
